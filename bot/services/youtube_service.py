...
# Import DatabaseManager for type hinting
...
class YouTubeService:
    ...
    async def get_video_details(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(
                f"Unexpected service error getting video info for {url}")
            raise ServiceError(
                f"Unexpected error getting video details: {type(e).__name__}")
    ...
    async def process_and_download(
        ...
        if quality_selector == 'audio':
            choice_description = "Audio Only"
        elif quality_selector == 'mp3':
            choice_description = "Audio Only (MP3)"
        elif quality_selector == 'gif':
            choice_description = "GIF (Full Video)"
        elif quality_selector == 'best':
            choice_description = "Best Quality Video"
        ...
        try:
            if status_callback:
                status_callback(
                    f"🚀 Starting download ({choice_description})...")
            ...
            if quality_selector == 'gif':
                ...
                try:
                    ...
                    logger.info(
                        f"GIF conversion successful: {gif_path} for record {download_record_id}")
                    final_media_path = gif_path  # Update final_media_path to the new GIF path
                    # Clean up original video used for GIF
                    ...
                    logger.error(
                        f"GIF conversion failed for record {download_record_id}: {conv_e}")
                    ...
                    logger.exception(
                        f"Unexpected error during GIF conversion for record {download_record_id}")
                    raise ConversionError(
                        f"Unexpected GIF conversion error: {type(e).__name__}") from e
            ...
            if not final_media_path or not os.path.exists(final_media_path):
                ...
                raise ServiceError(
                    f"Processed media file not found for record {download_record_id}.")

            logger.info(
                f"Processing complete for record {download_record_id}. Final path: {final_media_path}")
            ...
            logger.error(f"Service error for record {download_record_id}, URL {url}: {e}", exc_info=isinstance(
                e, ServiceError))
            ...
            logger.exception(
                f"Unexpected service error for record {download_record_id}, URL {url}")
            if downloaded_path and os.path.exists(downloaded_path):
                cleanup_file(downloaded_path)
            if final_media_path and final_media_path != downloaded_path and os.path.exists(final_media_path):
                cleanup_file(final_media_path)
            ...
            raise ServiceError(
                f"An unexpected error occurred during processing: {type(e).__name__}") from e

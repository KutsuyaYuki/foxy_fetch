...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        interaction_id = await db.log_interaction(
            ...
            content=trigger_message.text or trigger_message.caption or url_to_process # Log text or URL
        )
    ...
    try:
        ...
        await asyncio.sleep(0.1) # Slight delay cosmetic

        # --- Fetch video details (pass interaction_id for potential later use if needed) ---
        video_details = await youtube_service.get_video_details(url_to_process) # interaction_id not directly needed here yet
        # ----------------------------------------------------------------------------------
        title = video_details['title']
        duration = video_details['duration']
        formats = video_details['formats']

        # --- Check duration and formats ---
        if duration and duration > 3600 * 2: # Allow up to 2 hours now
             ...
             # Log failure potentially? Requires structure change or separate log event
             return
        if not formats:
        ...
        # --- Process formats and create keyboard ---
        ...
        # --- Edit status message with options ---
        ...
        # Log specific error if needed, depends on desired detail
        ...
        if status_message:
            ...
            # If status message failed to send, reply to original
...
# List of message handlers to register

...
import os
import time
# Add AbstractEventLoop to imports
from typing import Dict, Optional, Tuple, Any, Callable, TYPE_CHECKING

# Type hint for loop to avoid runtime import, only for static analysis
if TYPE_CHECKING:
...
async def download_media(
    url: str,
    format_choice: str,
    # Update callback signature to expect loop
    update_callback: Callable[[str, 'AbstractEventLoop'], None],
    # Add loop parameter here
    loop: 'AbstractEventLoop'
) -> Tuple[str, str]:
    """
    Downloads media based on the chosen format using yt-dlp,
    reporting progress via a callback with event loop context.
    """
        ...
        if d['status'] == 'downloading':
            try:
                ...
                if should_update:
                    ...
                    try:
                         # --- Pass the event loop to the callback ---
                         update_callback(progress_str, loop)
                         # -----------------------------------------
            ...
            if last_percentage != 100:
                try:
                     # Pass loop here too
                     update_callback("🚀 Downloading... 100%", loop)
                except Exception as cb_e:
    ...
    # --- Format selection logic (no changes) ---
    ...
    try:
        ...
        # Use the passed loop for executor, though default usually works too
        # loop = asyncio.get_running_loop() # Not needed, use passed loop

        def download_sync():
            nonlocal downloaded_file_path, final_file_path, info_dict
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info_dict = ydl.extract_info(url, download=True)
                # --- Determine final path logic (no changes) ---
        ...
        if is_merging_needed and final_file_path and os.path.exists(final_file_path):
            ...
            try:
                # Pass loop here too
                update_callback("⏳ Post-processing/Merging formats...", loop)
            except Exception as cb_e:
        ...
        if final_file_path and os.path.exists(final_file_path) and info_dict:
            ...
            # --- Error handling for file not found (no changes) ---

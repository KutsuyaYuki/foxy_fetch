...
async def get_video_info(url: str) -> Dict:
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched info with {len(info_dict.get('formats', []))} formats for {url}")
        return info_dict
    except yt_dlp.utils.DownloadError as e:
        err_msg = str(e)
        if "video unavailable" in err_msg.lower():
            raise DownloaderError("This video is unavailable.")
        elif "private video" in err_msg.lower():
            raise DownloaderError("This is a private video.")
        logger.error(f"yt-dlp DownloadError fetching info for {url}: {e}")
        raise DownloaderError(f"Could not get video info: {e.args[0]}")
    except Exception as e:
        logger.error(
            f"Unexpected error fetching info for {url}: {e}", exc_info=True)
        raise DownloaderError(
            f"Unexpected error fetching video info: {type(e).__name__}")


async def download_media(
    url: str,
    quality_selector: str,  # e.g., 'best', 'h720', 'audio', 'gif'
    # Progress hook is now optional
    ...
    try:
        logger.info(
            f"Starting yt-dlp download for {url} with selector '{quality_selector}'")
        loop = asyncio.get_running_loop()
        ...
        def download_sync():
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    path_keys = ['filepath', '_filename']
                    found_path = None
                    if info_dict:
                        if 'requested_downloads' in info_dict and info_dict['requested_downloads']:
                            found_path = info_dict['requested_downloads'][0].get(
                                'filepath')
                        if not found_path:
                    ...
                    if found_path and os.path.exists(found_path):
                        ...
                        if not final_file_path:
                            raise DownloaderError(
                                f"Determined path '{found_path}' but file not found with any extension.")
                    else:  # Should not happen if download succeeded
                        raise DownloaderError(
                            "yt-dlp finished, but could not determine file path.")

                    logger.info(
                        f"yt-dlp download sync finished. Final path: {final_file_path}")

            except yt_dlp.utils.DownloadError as ydl_err:
                raise DownloaderError(
                    f"Download failed: {ydl_err.args[0]}") from ydl_err
            except Exception as sync_err:  # Catch other sync errors
                logger.exception("Error during synchronous download part")
                raise DownloaderError(
                    f"Sync download error: {sync_err}") from sync_err
        ...
        if final_file_path and os.path.exists(final_file_path) and info_dict:
            ...
            raise DownloaderError(
                "Download process ok, but final file path invalid or missing.")
        ...
        if final_file_path:
            cleanup_file(final_file_path)  # Cleanup any partial/guessed file
        raise  # Re-raise the specific DownloaderError
    except Exception as e:  # Catch unexpected errors (e.g., loop errors)
        logger.exception(
            f"Unexpected error during download media call for {url}: {e}")
        if final_file_path:
            cleanup_file(final_file_path)
        raise DownloaderError(
            f"Unexpected download error: {type(e).__name__}") from e
